...
ambulance_df = pd.read_csv('2020_firetruck_data.csv')
...
weather_df = pd.read_csv('NOAA_DATA.csv')
weather_df['DATE'] = pd.to_datetime(weather_df['DATE'])  # remember to convert to standard format
merged_df['DATE'] = pd.to_datetime(merged_df['INCIDENT_DATETIME'])
merged_df = pd.merge_asof(merged_df.sort_values('DATE'), weather_df.sort_values('DATE'),
                          on='DATE', direction='nearest')

# select the relevant columns for the tensor
temperature = merged_df['TMIN'].values.astype(float).reshape(-1, 1)
humidity = merged_df['SNWD'].values.astype(float).reshape(-1, 1)
precipitation = merged_df['PRCP'].values.astype(float).reshape(-1, 1)
print(temperature)
...
array = t.numpy()
np.savetxt("firetrucks_big.csv", array, delimiter=",")
