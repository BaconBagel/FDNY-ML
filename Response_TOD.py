...
ny_map = folium.Map(location=[40.7128, -74.0060], zoom_start=10, attr="stamen")
df[0] = df[0] + 6



# Define function to convert Unix timestamp to 24-hour time format
def unix_to_24h_time(unix_timestamp):
    dt_object = datetime.datetime.fromtimestamp(unix_timestamp)
    return dt_object.strftime('%H')


# Apply function to Timestamp column
df[0] = df[0].apply(unix_to_24h_time)
df[0] = pd.to_numeric(df[0], errors='coerce')
# filter dataframe for negative values in the fifth column
neg_df = df[(df[6] > 238) & (df[0] > 11)]
neg_df2 = df[(df[6] > 238) & (df[3] <= 11)]
...
pos_df =df[(df[6] < 238) & (df[0] > 11)]
pos_df2 =df[(df[6] < 238) & (df[3] <= 11)]
...
ny_map.save('resp_time_tod.html')
